...
# Open hapcount table into R, put name of the file in "file name"
hapcount_table <- read.table("file name", 
...
#Remove data with NA recombination
...
# Remove the centromere, chromosme dependant. In this case for chromosome 1
...
for(n in loop.vector.1){
  ...
  # Open inner loop for 1 row. Each row represents one 20kb window with recombination data  
  for(f in loop.vector.2){  #For each collumn
    ...
    # Define hapfreq and multiplicity vectors for 1 window
    hapfreq[f-6] <- d[2]
    multiplicity[f-6] <- d[1]
     
  } #Done the calculation in every collumn for a single window
  ...
  # Define 2 most common haplotypes for 1 window 
  hapfreq1 <- 0
  hapfreq2 <- 0
  ...
  #If most common haplotype was 2:16 then obtain hapfreq2 again from the 2:16 since there are two haplotypes with equal number of people
  else {hapfreq2 <- hapfreq[1]}
  if(is.na(hapfreq2)) {hapfreq2 <- 0}
  
  # Calculate H1, H12, H2 and H2/H1 for all windows
...
#Plot H1, H12 and H2/H1 against recombination rate.
